...
if [ "$cygwin" = "true" -o "$msys" = "true" ] ; then
    ...
    ROOTDIRSRAW=`find -L / -maxdepth 2 -name local$ -prune -o -type d -print 2>/dev/null`
    SEP=""
...
for arg in "$@" ; do
    case $arg in
        -*.gradle.user.home=* ) USER_HOME_OPT="${arg#-*.gradle.user.home=}" ;;
        /* )
            if [ -d "$arg" ]; then
                USER_HOME_OPT="$arg"
            fi
...
case $((80 - i)) in
 *1*)
    # echo some value each time around the loop
    i=$(($i+1))
    ;;
esac
